...
UltimateRAG 同时是学习型项目和企业级工程参考实现，因此生产代码必须提供足够详细、可维护的文档注释。

必须遵守：

1. 每个模块应使用模块级 Docstring 说明模块职责及其架构边界
2. 每个核心类、领域接口、基础设施适配器和应用服务必须说明其职责，以及它不负责什么
3. 公共函数和公共方法必须说明行为；当参数、返回值或异常并非一目了然时，应明确说明
4. 关键业务流程必须解释状态变化、一致性选择、安全约束和失败行为背后的原因
5. 外部服务适配器必须说明外部协议、批量限制、超时或数据映射等重要约束
6. 测试中的关键场景应通过清晰测试名称或简短注释说明所验证的行为
...
async def ingest(
    ...
    """同步摄取一份文档，并仅在向量索引成功后将其标记为 READY。

    原始文件会先持久化，以便处理失败后仍可排查和重建索引。

    Args:
        knowledge_base_id: 文档所属知识库 ID。
        filename: 仅用于展示和类型判断的原始文件名。
        content: 已完成上传大小限制检查的原始字节。

    Returns:
        完成处理后的文档领域对象。

    Raises:
        InvalidDocumentError: 文件类型、编码或内容不符合要求。
        DocumentProcessingError: 解析、向量化或索引阶段失败。
    """
```

详细不等于重复。禁止为每一行代码添加解释性噪音，也不要编写与实现不一致、无法长期维护的注释。
