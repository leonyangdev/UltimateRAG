...
Open PDF
  ↓
PDFium page text probe
  ├── 低文字量页 → Render JPEG → 百炼 OCR → IMAGE Block
...
这种按页判定支持“前几页原生文本、后几页扫描图”的混合 PDF。Docling 在 Worker 本地运行并关闭
...
要求提取可见文字、图表数值/趋势、流程节点和箭头关系，不允许猜测。单张附图理解失败时降级为
...
2. `TABLE` 按行装箱，每个 Chunk 重复 Markdown 表头。
...
- 跨页 Block 不被错误合并
- 同页 BBox 合并、表格切分重复表头与严格 Token 上限
...
名称、带 Locator 的检索与流式 Citation，最后清理临时知识库。
